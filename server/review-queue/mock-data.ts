...
export function getReviewQueueMockData(): ReviewQueueRecord[] {
  return [
    ...
    {
      id: "post_x_launch_published",
      articleId: "article_launch_published",
      platform: SocialPlatform.X,
      tone: GenerationTone.BOLD,
      ...
      generationModel: "gpt-5.4-mini",
      generatedText:
        "REZZUM keeps the path from RSS item to published social post visible all the way through delivery. That matters when you need reliable publishing, not just more draft volume.",
      editedText:
        "Publishing is only useful when the workflow stays traceable.\n\nREZZUM keeps the path from feed item to approved draft to live post visible, so teams can move fast without losing control.",
      generationStartedAt: hoursAgo(15),
      generatedAt: hoursAgo(15),
      reviewedAt: hoursAgo(14),
      approvedAt: hoursAgo(13),
      rejectedAt: null,
      scheduledFor: hoursAgo(5),
      ...
      updatedAt: hoursAgo(4),
      article: {
        id: "article_launch_published",
        title: "Why visible publishing state matters once AI-generated content reaches production",
        sourceEntryId: "publish-state-launch",
        sourceUrl: "https://signals.example/ai/publish-state",
        canonicalUrl: "https://signals.example/ai/publish-state",
        contentHash: "hash_publish_state",
        excerpt:
          "A practical look at why post status, publish timing, and failure handling need to stay visible in AI-assisted systems.",
        contentText:
          "Teams often focus on generation quality and overlook what happens after approval. In production, the actual publishing state is where trust is won or lost. Operators need to know when a post is still editable, when it is committed to a future send, when it has been delivered, and what failed if a publish attempt did not complete.",
        authorName: "Lena Ortiz",
    ...
    {
      id: "post_linkedin_failure_case",
      articleId: "article_failure_case",
      platform: SocialPlatform.LINKEDIN,
      tone: GenerationTone.PROFESSIONAL,
      ...
      generationModel: "gpt-5.4",
      generatedText:
        "A mature publishing system treats failed sends as a first-class state. Operators need the failure reason, the attempted schedule, and a safe path to retry.",
      editedText:
        "A reliable publishing system does not hide delivery failures.\n\nTeams need the attempted time, the provider response, and enough context to fix the issue without guessing what happened.",
      generationStartedAt: hoursAgo(19),
      generatedAt: hoursAgo(19),
      reviewedAt: hoursAgo(18),
      approvedAt: hoursAgo(17),
      rejectedAt: null,
      scheduledFor: hoursAgo(2),
      publishedAt: null,
      failedAt: hoursAgo(2),
      failureReason: "LinkedIn returned an authorization error during the last publish attempt.",
      versionNumber: 1,
      createdAt: hoursAgo(19),
      updatedAt: hoursAgo(2),
      article: {
        id: "article_failure_case",
        title: "Delivery failure handling is part of the publishing UX, not an edge case",
        sourceEntryId: "failure-handling-publishing",
        sourceUrl: "https://rezzum.example/blog/failure-handling",
        canonicalUrl: "https://rezzum.example/blog/failure-handling",
        contentHash: "hash_failure_case",
        excerpt:
          "Why status visibility and retry-ready error details are central to operational publishing tools.",
        contentText:
          "The weakest publishing systems are the ones that collapse all delivery problems into a generic failure label. Stronger systems keep the scheduled time, provider context, and the exact failure reason visible so a human can recover the workflow without manually reconstructing what happened.",
        authorName: "REZZUM Team",
        publishedAt: hoursAgo(30),
        discoveredAt: hoursAgo(24),
        status: ArticleStatus.PROCESSED,
        filteredOutReason: null,
        feed: {
          id: "feed_rezzum_updates",
